...
def apply_filters(df, filters):
    ...
    if filters.get("fatura_no"):
        subset = subset[
            subset["fatura_no"].astype(str).str.lower() == str(filters["fatura_no"]).lower()
        ]

    if filters.get("cari_filter"):
        subset = subset[
            subset["cari_kod"].astype(str).str.lower() == str(filters["cari_filter"]).lower()
        ]

    if filters.get("stok_filter"):
        subset = subset[
            subset["stok_kod"].astype(str).str.lower() == str(filters["stok_filter"]).lower()
        ]

    if filters.get("cari_ad_filter"):
        subset = subset[
            subset["cari_ad"].astype(str).str.contains(filters["cari_ad_filter"], case=False, na=False)
        ]

    if filters.get("urun_filter"):
        subset = subset[
            subset["urun_adi"].astype(str).str.contains(filters["urun_filter"], case=False, na=False)
        ]
